...
export class TutorService {
  ...
  async searchTutors(params: {
    ...
    // If geo params are provided, add a $near filter
    const hasGeo =
    ...
    // Fetch user data for all tutors
    const userIds = profiles.map((profile) => profile.userId);
    console.log('[TutorService.searchTutors] Fetching user data for userIds:', userIds);
    ...
    console.log('[TutorService.searchTutors] Found users:', users);

    // Create a map for quick user lookup
    const userMap = new Map(users.map((user) => [user.id, user]));

    // Map to response DTOs and calculate distances when geo is present
    return profiles.map((profile) => {
      ...
      if (user) {
        console.log(`[TutorService.searchTutors] Adding user ${user.id} name=${user.name} email=${user.email} to tutor ${profile._id}`);
        dto.name = user.name;
        dto.email = user.email;
      } else {
        console.log(`[TutorService.searchTutors] User not found for tutor ${profile._id} with userId ${profile.userId}`);
      }
