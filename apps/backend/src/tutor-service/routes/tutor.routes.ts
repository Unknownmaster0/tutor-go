...
export const createTutorRoutes = (tutorController: TutorController): Router => {
  ...
  router.put('/profile', authenticateToken, updateTutorProfileValidation, asyncHandler(tutorController.updateProfile));
  ...
  // Video upload routes
  router.post('/upload-video', authenticateToken, upload.single('video'), asyncHandler(tutorController.uploadVideo));
  router.delete('/delete-video', authenticateToken, asyncHandler(tutorController.deleteVideo));

  // Availability management routes
  router.put('/availability', authenticateToken, setAvailabilityValidation, asyncHandler(tutorController.setAvailability));
  router.post(
    '/availability/slot',
    authenticateToken,
    [
