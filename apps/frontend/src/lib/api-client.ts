  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        ...
        // Handle network errors (gateway down, no connection, etc.)
        if (!error.response) {
          const networkError = new Error(
            'Network Error: Unable to reach the server. Please check your connection and ensure the API Gateway is running on http://localhost:3001'
          );
          console.error('Network Error:', error.message);
          return Promise.reject(networkError);
        }

        // Handle 503 Service Unavailable (service down)
        if (error.response?.status === 503) {
          const serviceError = new Error(
            `Service Unavailable: ${error.response.data?.message || 'The requested service is currently unavailable. Please try again later.'}`
          );
