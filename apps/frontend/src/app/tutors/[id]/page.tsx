  ...
  useEffect(() => {
    const fetchTutorProfile = async () => {
      try {
        ...
        // Fetch reviews using userId (the reviews endpoint expects a UUID, not MongoDB ID)
        try {
          const reviewsData = await apiClient.get<{ reviews: Review[]; total: number }>(
            `/reviews/tutor/${tutorData.userId}`
          );
          setReviews(reviewsData.reviews || []);
        } catch (reviewError) {
          // Reviews endpoint might fail, but we still show the tutor profile
