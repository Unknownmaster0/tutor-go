...
export default function SearchPage() {
  ...
  const fetchTutors = useCallback(async (pageNum: number = 1, currentFilters: FilterValues = {}) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const response = await apiClient.get<{ tutors: TutorProfile[]; total: number; page: number; limit: number }>(
        `/tutors/search?${queryString}`
      );
      ...
      setHasMore(response.tutors.length === response.limit);
    } catch (err: any) {
