    ...
    if (!filtersApplied.current && tutors.length === 0) {
      ...
      (async () => {
        ...
        try {
          ...
          const tutorsData = await apiClient.get<TutorProfile[]>('/tutors/top-rated?limit=8');
          // Calculate distances if user location is available
          const tutorsWithDistance = tutorsData.map((tutor: TutorProfile) => {
            let distance = undefined;
                ...
                <div className="relative bg-gray-200 w-full" style={{ minHeight: '400px' }}>
                  {tutors.length > 0 ? (
                    <TutorMap
                      ...
                      userLocation={
                        userLat && userLng ? { latitude: userLat, longitude: userLng } : undefined
                      }
                    />
                  ) : (
